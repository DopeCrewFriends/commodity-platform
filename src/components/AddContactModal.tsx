...
const AddContactModal: React.FC<AddContactModalProps> = ({ walletAddress, onClose }) => {
  ...
  useEffect(() => {
    if (!isOnline || !searchQuery.trim() || searchQuery.length < 2) {
      setSearchResults([]);
      return;
    }
    ...
    <div className="wallet-modal active" onClick={onClose}>
      ...
      <div className="wallet-modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="wallet-modal-header">
          <h2>Add Contact</h2>
          <p>Search by username or add manually</p>
        </div>
        <form onSubmit={handleSubmit}>
          {!selectedUser && !manualMode && (
            <div style={{ marginBottom: '1rem' }}>
                  ...
                  {searchResults.map((user) => (
                    ...
                    >
                      <div style={{ fontWeight: 'bold' }}>
                        {user.name} {user.username && <span style={{ color: '#666', fontSize: '0.9em' }}>@{user.username}</span>}
                      </div>
              ...
              {searchQuery && searchResults.length === 0 && !isSearching && isOnline && (
